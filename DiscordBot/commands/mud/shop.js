...
class ShopCommand extends commando.Command {
  ...
  constructor(client) {
    super(client, {
      ...
      group: 'mud',
      memberName: 'shop',
      description: ShopCommand.commandInfo(),
      args: [
        {
          key: 'npc',
          prompt: 'Who are you Shoping to?',
          type: 'string'
        }
      ]
    });
  ...
  getNPC(message, player, room, entity) {
    if (!player.busy) {
      if (room.npcs[entity]) {
        ...
        // if not an npc, determine if the player is Shoping to an enemy
        if (room.enemies[entity]) {
          // if an enemy, then mention that the player can't Shop to the enemy
          message.channel.send(`${player.characterName} fails an attempt at parley with the ${entity}`);
        } else {
          // if not an enemy either, they're Shoping to no one
  ...
  determineResponse(npc, player) {
    ...
    if (playerProgress) {
      ...
      // if the player hasn't Shoped to this NPC before, create the player's progress in their object
